...
static IRAM_ATTR void wifi_sniffer_packet_handler(void *buff, wifi_promiscuous_pkt_type_t type)
{
    ...
    if ((wifi_rssi_threshold) && (rssi < wifi_rssi_threshold)) // rssi is negative value
        return;

    mac_add((uint8_t *)hdr->addr2, MAC_SNIFF_WIFI);

    if (libpax_mac_callback) {
        // frame_ctrl: type bits [3:2], subtype bits [7:4]
        const uint16_t fc = hdr->frame_ctrl;
        const unsigned frame_type = (fc >> 2) & 0x3;
...
void wifi_sniffer_init(uint16_t wifi_channel_switch_interval)
{
    ...
    if (wifi_channel_switch_interval > 0) {
        WifiChanTimer = xTimerCreate("WifiChannelTimer", pdMS_TO_TICKS(wifi_channel_switch_interval * 10), pdTRUE, (void *)0,
                                     switchWifiChannel);
...
void wifi_sniffer_stop()
{
    ...
    if (initialized_wifi) {
        ...
        ESP_ERROR_CHECK(esp_wifi_set_promiscuous(false)); // now switch off monitor mode
        esp_wifi_deinit();
        initialized_wifi = 0;
    }
#endif
