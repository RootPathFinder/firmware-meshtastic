...
int counter_mode;
libpax_mac_callback_t libpax_mac_callback = NULL;

void libpax_set_mac_callback(libpax_mac_callback_t cb)
{
    libpax_mac_callback = cb;
}
...
int libpax_deserialize_config(char *source, struct libpax_config_t *configuration)
{
    ...
    if (storage_buffer.minor_version != CONFIG_MINOR_VERSION) {
        ESP_LOGW("libpax", "Restoring config with different MINOR version: %d.%d instead of %d.%d", storage_buffer.major_version,
                 storage_buffer.minor_version, CONFIG_MAJOR_VERSION, CONFIG_MINOR_VERSION);
    }
...
int libpax_update_config(struct libpax_config_t *configuration)
{
    ...
    if (configuration->wificounter) {
        ESP_LOGE("libpax", "Configuration requests Wi-Fi but was disabled at compile time.");
    ...
    if (configuration->blecounter) {
        ESP_LOGE("libpax", "Configuration requests BLE but was disabled at compile time.");
...
int libpax_counter_init(void (*init_callback)(void), struct count_payload_t *init_current_count,
                        uint16_t init_pax_report_interval_sec, int init_counter_mode)
{
    ...
    PaxReportTimer =
        xTimerCreate("PaxReportTimer", pdMS_TO_TICKS(init_pax_report_interval_sec * 1000), pdTRUE, (void *)0, report);
